...
export default function Navbar({ isMobileMenuOpen, setIsMobileMenuOpen, onBuyTickets, hasOfferBanner }) {
  ...
  return (
    <>
      {/* ──── TOP NAVIGATION ──── */}
      <nav className={`fixed ${hasOfferBanner ? 'top-0 md:top-[40px]' : 'top-0'} w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/90 dark:bg-surface-container-lowest/90 backdrop-blur-xl shadow-lg shadow-black/5 dark:shadow-black/20' : 'bg-transparent'}`}>
